...
export const validateScoreSubmission = (submission: ScoreSubmission) => {
  const shareCode = normalizeShareCode(submission.shareCode);

  if (!isValidShareCode(shareCode)) {
    return { ok: false as const, error: 'Invalid challenge code.' };
  }

  if (submission.mode !== 'tail' && submission.mode !== 'type') {
    return { ok: false as const, error: 'Invalid game mode.' };
  }

  if (!Array.isArray(submission.answers) || submission.answers.length !== 5) {
    return { ok: false as const, error: 'A completed game must include five answers.' };
  }
  ...
  for (const [index, question] of questions.entries()) {
    ...
    if (!answer || answer.questionId !== question.id) {
      return { ok: false as const, error: 'Submitted answers do not match this challenge.' };
    }

    if (answer.correctAirlineId !== question.airlineId) {
      return { ok: false as const, error: 'Answer metadata does not match this challenge.' };
    }

    if (!Number.isFinite(answer.elapsedMs) || answer.elapsedMs < 0 || answer.elapsedMs > QUESTION_TIME_MS) {
      return { ok: false as const, error: 'Answer timing is invalid.' };
    }

    const airline = airlinesById.get(question.airlineId);

    if (!airline) {
      return { ok: false as const, error: 'Unknown airline in challenge.' };
    }
    ...
    if (
      ...
      return { ok: false as const, error: 'Submitted score does not match verified answers.' };
    }
  ...
  if (submission.totalScore !== validatedTotal) {
    return { ok: false as const, error: 'Submitted total score is incorrect.' };
  }

  return {
    ok: true as const,
    shareCode,
    mode: submission.mode as GameMode,
    totalScore: validatedTotal,
    answers: submission.answers as PlayerAnswer[],
  };
};
