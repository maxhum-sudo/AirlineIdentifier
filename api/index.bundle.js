...
var fetchLeaderboard = async (input) => {
  await ensureSchema();
  const sql = getSql();
  const limit = Math.min(Math.max(input.limit ?? 20, 1), 50);
  const rows = input.shareCode ? input.mode ? await sql`
...
var fetchRankForScore = async (input) => {
  await ensureSchema();
  const sql = getSql();
  const rows = input.shareCode ? await sql`
...
// shared/session.ts
var SHARE_CODE_PATTERN = /^[A-Z0-9-]{4,32}$/;
var normalizeShareCode = (shareCode) => shareCode.trim().toUpperCase().replace(/[^A-Z0-9]/g, "").slice(0, 12);
var isValidShareCode = (shareCode) => SHARE_CODE_PATTERN.test(normalizeShareCode(shareCode));
var sanitizePlayerName = (value) => value.trim().replace(/\s+/g, " ").slice(0, 24);

// shared/airlineSeeds.ts
var airlineSeeds = [
  ...
  { id: "azul", name: "Azul Brazilian Airlines", country: "Brazil", iata: "AD", icao: "AZU", categoryTitle: "Category:Azul Linhas A\xE9reas Brasileiras aircraft tailfins" },
  { id: "gol", name: "GOL Linhas A\xE9reas", country: "Brazil", iata: "G3", icao: "GLO", categoryTitle: "Category:Gol Transportes A\xE9reos aircraft tailfins" },
  { id: "aerolineas", name: "Aerol\xEDneas Argentinas", country: "Argentina", iata: "AR", icao: "ARG", categoryTitle: "Category:Aerol\xEDneas Argentinas aircraft tailfins" },
...
var buildValidationAirlines = () => airlineSeeds.map((airline) => ({
  id: airline.id,
  name: airline.name,
...
// shared/questions.ts
var ANSWER_OPTION_COUNT = 4;
var buildQuestionSet = (airlines, seed, roundCount, mode = "tail") => {
  const candidates = airlines.filter(
    (airline) => airline.images.some((image) => image.category === "tail")
  );
  ...
  return selectedAirlines.map((airline, index) => {
    const optionAirlineIds = mode === "type" ? [] : shuffleSeeded(
      [
        airline.id,
        ...shuffleSeeded(
          candidates.filter((candidate) => candidate.id !== airline.id),
          `${seed}:${airline.id}:options`
        ).slice(0, ANSWER_OPTION_COUNT - 1).map((candidate) => candidate.id)
      ],
...
// shared/scoring.ts
var QUESTION_TIME_MS = 1e4;
var BASE_CORRECT_POINTS = 100;
var MAX_TIME_BONUS = 100;
var calculateQuestionScore = ({
  isCorrect,
  elapsedMs,
...
var normalizeAnswer = (value) => value.trim().toLowerCase().replace(/[^\w\s]/g, "").replace(/\s+/g, " ");
var isTypedAnswerCorrect = (input, airline) => {
...
// shared/validateScore.ts
var validationAirlines = buildValidationAirlines();
var airlinesById = new Map(validationAirlines.map((airline) => [airline.id, airline]));
var validateScoreSubmission = (submission) => {
  ...
  for (const [index, question] of questions.entries()) {
    ...
    if (answer.isCorrect !== isCorrect || answer.score !== expectedScore || answer.correctAirlineId !== question.airlineId) {
...
var handlePostScores = async (request) => {
...
var handleGetLeaderboard = async (request) => {
  if (request.method !== "GET") {
    return jsonResponse(405, { error: "Method not allowed." });
  }
  const url = new URL(request.url);
  const modeParam = url.searchParams.get("mode");
  const mode = modeParam === "tail" || modeParam === "type" ? modeParam : void 0;
  const shareCode = url.searchParams.get("shareCode") || void 0;
...
// lib/api/vercel.ts
var applyCorsPreflightIfNeeded = (request, response) => {
  if (request.method !== "OPTIONS") {
    return false;
  }
...
var toWebRequest = (request, fallbackPath, includeBody = false) => {
  ...
  return new Request(url, {
    ...
    body: includeBody && request.method === "POST" ? JSON.stringify(request.body ?? {}) : void 0
  });
};
